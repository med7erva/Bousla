...
const PlaceholderPage: React.FC<{ title: string; icon: any }> = ({ title, icon: Icon }) => (
  <div className="flex flex-col items-center justify-center h-[60vh] text-center p-8 bg-white dark:bg-slate-800 rounded-3xl border border-dashed border-gray-300 dark:border-slate-700">
    ...
    <p className="text-gray-500 dark:text-slate-400 max-w-md">جاري العمل على هذه الصفحة. ستكون متاحة قريباً.</p>
  </div>
);
    ...
    return (
        <div className="min-h-screen bg-slate-50 dark:bg-slate-900 flex items-center justify-center p-4 font-sans" dir="rtl">
            <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl shadow-xl max-w-md w-full text-center border border-red-100 dark:border-red-900/30">
                <div className="w-20 h-20 bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 rounded-full flex items-center justify-center mx-auto mb-6"><AlertTriangle size={40} /></div>
                <h2 className="text-2xl font-black text-slate-900 dark:text-white mb-4">انتهت صلاحية الحساب</h2>
                <p className="text-slate-600 dark:text-slate-400 mb-8 leading-relaxed">عذراً، لقد انتهت الفترة التجريبية أو مدة اشتراكك الحالي. يرجى التجديد للمتابعة في استخدام نظام بوصلة.</p>
                <div className="space-y-3">
                    <Link to="/pricing" className="flex items-center justify-center gap-2 bg-emerald-600 text-white py-4 rounded-2xl font-bold hover:bg-emerald-700 transition shadow-lg shadow-emerald-200">عرض خطط الاشتراك</Link>
                    <Link to="/profile" className="flex items-center justify-center gap-2 text-slate-500 dark:text-slate-400 py-2 font-bold hover:text-slate-800 transition"><span>لديك كود تفعيل؟ أدخله هنا</span><ArrowRight size={16} className="rotate-180" /></Link>
                </div>
            </div>
        </div>
    );
    ...
    if (user?.subscriptionPlan !== 'pro' && !user?.isAdmin) {
        return (
            <div className="flex flex-col items-center justify-center h-[60vh] text-center p-8 bg-white dark:bg-slate-800 rounded-3xl border border-indigo-100 dark:border-indigo-900/30">
                <div className="w-20 h-20 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 rounded-full flex items-center justify-center mb-6"><Lock size={40} /></div>
                <h2 className="text-2xl font-bold text-slate-800 dark:text-white mb-2">ميزة حصرية لعملاء Pro</h2>
                <p className="text-slate-500 dark:text-slate-400 max-w-md mb-6">هذه الميزة متاحة فقط في خطة الأعمال (Pro). قم بترقية حسابك الآن للاستفادة منها.</p>
                <Link to="/pricing" className="bg-indigo-600 text-white px-8 py-3 rounded-xl font-bold hover:bg-indigo-700 transition shadow-lg">ترقية الآن</Link>
            </div>
        );
    }
    return <>{children}</>;
